% Function: execute the hyper opponent transformation on the 67 class indoor scene dataset
% Author: Yang Xiao @ C2I SCE NTU (hustcowboy@gmail.com)
% Tips: The 67 class indoor scene dataset can be downloaded at http://web.mit.edu/torralba/www/indoor.html
...
nSamImg = 20;%80;                 % number of sampling images per round
...
directory_name = uigetdir('F:\mCENTRIST_v1.0\mCENTRIST_v1.0\101_ObjectCategories.tar','Mixed Images');
...
% trans_matrix = [0.3669 0.3358 0.2974; -0.4514 0.0638 0.4848; -0.2279 0.4947 -0.2774]';  
%------------------------------------------------------------
...
rgb_ncount = cell(1, length(subfolder)-2);    % RGB image num counter for each class
for i = 1:length(subfolder)-2
    rgb_ncount{1,i} = struct;
    rgb_ncount{1,i}.num = 0;
end
...
for i = 1:length(subfolder)
    if ~strcmp(subfolder(i,1).name,'.') && ~strcmp(subfolder(i,1).name,'..')
        ...
        rgb_ncount{1, i-2}.class = subfolder(i,1).name;    % class name
        ...
        for j = 1:length(file)
            ...
            if isempty(map)
                if nChannel == 3
                    ...
                    ncount = ncount + 1;
                    rgb_ncount{1,i-2}.num = rgb_ncount{1,i-2}.num + 1;
                else
                    continue;
                end
            else
            ...
            save_filename = '00000';
            fileidx = num2str(rgb_ncount{1,i-2}.num);
            for k = 1:length(fileidx)
                save_filename(end-k+1) = fileidx(end-k+1);
            end
            ...
            I_filter = rgb_img(:,:,1);  sobel = gradimg_obtain(I_filter, 1);    sobel = floor(mat2gray(sobel) * 255); % normalize Sobel image within [0 255]
