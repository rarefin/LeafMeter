% Function: scene categorization on the 21 class land-use classification dataset using mCENTRIST 
...
ts_num_cat = 20;               % number of test examples per category 
...
% img_dir ={'image/land-use 21 class/O1', 'image/land-use 21 class/O2', 'image/land-use 21 class/O3',...
%      'image/land-use 21 class/Sobel_R'};       % directory for the image database  
% img_dir ={'image/land-use 21 class/K'};
img_dir ={'image/UCMerced_LandUse/Images'};

data_dir = 'data/UCMerced_LandUse/mCENTRIST/O1O2O3Sobel_m1';           % directory for saving mCENTRIST descriptors
split_dir = 'sample_split/UCMerced_LandUse';                                                % directory for training and test sample split
...
%     ts_idx = sam_split.ts_split{1,ii};
    ...
    % generate sample split randomly (five-fold cross validation)
    if ii == 1  %first round
        for jj = 1:nclass,
            idx_label = find(database.label == clabel(jj));
            num = length(idx_label);
            rng(ii+jj);
            idx_rand{jj} = randperm(num);
            
            ts_idx = [ts_idx; idx_label(idx_rand{jj}(1:ts_num_cat))];
            tr_idx = [tr_idx; idx_label(idx_rand{jj}(ts_num_cat+1:end))];
            
        end
    else
        for jj = 1:nclass,
            idx_label = find(database.label == clabel(jj));
            ts_idx = [ts_idx; idx_label(idx_rand{jj}((ii-1)*ts_num_cat+1:ii*ts_num_cat))];
            tr_idx = [tr_idx; [idx_label(idx_rand{jj}(1:(ii-1)*ts_num_cat)); idx_label(idx_rand{jj}(ii*ts_num_cat+1:end))] ];
        end
    end
