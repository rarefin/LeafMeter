% Function: execute the hyper opponent transformation on the 8 class sports event dataset
% Author: Yang Xiao @ C2I SCE NTU (hustcowboy@gmail.com)
% Tips: The  8 class sports event dataset can be downloaded at http://vision.stanford.edu/lijiali/event_dataset
...
nRounds = 1;                    % number of rounds of image sampling on the dataset
nSamImg = 1;                 % number of sampling images per round
nSamPixel = 1500;            % number of sampling pixels in each multi-channel image ********SAME
nChannel = 3;                   % number of image channels

%% Path setting
% obtain the root path of the RGB images
% directory_name = uigetdir('F:\mCENTRIST_v1.0\mCENTRIST_v1.0\event_dataset\','Mixed Images');
% indx = strfind(directory_name,'\');
% directory_name_top = directory_name(1:indx(end)-1);
directory_name = 'E:\OZONE_METER\Leaf_Sample';
directory_name_top = 'E:\OZONE_METER';
cov_dir = 'cov';       % path for saving the covariance matrices
path_cov = fullfile(directory_name_top, cov_dir);
if exist(path_cov, 'dir') ~= 7
    mkdir(path_cov);
end

vv_dir = 'vec_val';   % path for saving the eigenvector and eigenvalue
...
% trans_matrix = [0.3322 0.3418 0.3260; 0.4007 0.0933 -0.5060; 0.3345 -0.4912 0.1742]';  
%------------------------------------------------------------
